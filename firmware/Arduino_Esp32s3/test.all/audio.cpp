...
void test_audio() {
  int16_t data[DMA_BUF_LEN];
  record(data);
  int16_t mean = calculate_mean(data, DMA_BUF_LEN);
  log_info("Mean: %d", mean);
  if (mean > SOUND_THRESHOLD * 5) {
    set_led(COLOR_RED, 60);
    play(data, DMA_BUF_LEN, 1.0 * 5);
    delay(1000);
    stop_play();
    delay(500);
  } else {
